...
s = m:section(TypedSection, "system", translate("System Properties"), translate("ПОМНИ ! для перезапуска Zram с нужными значениями, сначала примени их ! ! !"))
s.anonymous = true
s.addremove = false
...
s:tab("zram",  translate("Настройки Zram"))
...
e = s:taboption("zram", Flag, "_init", translate("Активировать Zram"))
e.rmempty  = false

function e.cfgvalue(self, section)
	return luci.sys.init.enabled("zram") and self.enabled or self.disabled
end

function e.write(self, section, value)
	if value == "1" then
		luci.sys.call("/etc/init.d/zram enable >/dev/null")
		luci.sys.call("/etc/init.d/zram start >/dev/null")
	else
		luci.sys.call("/etc/init.d/zram stop >/dev/null")
		luci.sys.call("/etc/init.d/zram disable >/dev/null")
	end
end

o = s:taboption("zram", ListValue, "zram_size_mb", translate("Размер подкачки Zram"), "MB")
o:value(3)
o:value(4)
o:value(6)
o:value(8)
...
start = s:taboption("zram", Button, "apply", translate("Перезагрузка Zram"))
start.inputstyle = "apply"
start.write = function(self, section)
	luci.sys.call("/etc/init.d/zram stop >/dev/null && sleep 2 && /etc/init.d/zram start >/dev/null")
		end
...
o = s:taboption("logging", Value, "log_file", translate("Системный журнал в файл"))
o.optional    = false
o.placeholder = "/usr/log/log.txt"
